...
<html lang="en">
    ...
    <body>
        <div class="mx-96 mt-5">
            ...
            <div class="border-b border-zinc-300 flex items-center justify-center text-zinc-800 mt-1 pb-2">
                <div>
                    @if(count($stories) != 0)
            ...
            <ol>
                @foreach ($stories as $story)
                    <li class="border-b border-zinc-300 hover:border-zinc-500 p-3">
                        <div class="mb-1">
                            @if( isset($story['url']) )
                                <span class="text-lg">
                                    <a href="{{ route('story', ['id' => $story['id']]) }}" target="_blank">{{ $story['title'] }}</a>
                                </span>
                                <br>
                            ...
                            @else
                                <a class="text-lg" href="{{ route('story', ['id' => $story['id']]) }}">{{ $story['title'] }}</a>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center text-sm">
                            ...
                            <span class="inline-flex hover:bg-neutral-200 rounded-lg mx-1">
                                <a class="inline-flex items-center pl-0.5 pr-1" href="{{ route('user', ['id' => $story['by']]) }}">
                            ...
                            <span class="inline-flex items-center mx-1">
                                ...
                                <p class="ml-0.5">{{ HomeController::getFormatedTime(time() - $story['time']) }} ago</p>
                            </span>

                            <span class="inline-flex hover:bg-neutral-200 rounded-lg mx-1">
                                <a class="inline-flex items-center pl-0.5 pr-1" href="{{ route('story', ['id' => $story['id']]) }}"">
                                    ...
                                    @if(!isset($story['descendants']) || $story['descendants'] == 1)
                                        <p class="ml-1">1 comment</a></p>
                                    @else
                                        <p class="ml-1">{{ $story['descendants'] }} comments</p>
                                    @endif  
                                </a>
                            </span>
                        </div>
